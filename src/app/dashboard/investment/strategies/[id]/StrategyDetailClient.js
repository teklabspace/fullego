...
// Shared detail view. `strategyId` comes from the path route ([id]) for the
// statically exported platform strategies, or from the ?id= query route
// (strategies/detail) for user-created/cloned strategies — the query route is
// what makes arbitrary ids work under `output: 'export'`.
export default function StrategyDetailClient({ strategyId }) {
  const { isDarkMode } = useTheme();
  const params = { id: strategyId };
...
// Render a strategy parameter value readably — arrays join with commas,
// nested objects become "key: value" pairs instead of "[object Object]".
function formatParameterValue(value) {
  ...
  if (typeof value === 'object') {
    return Object.entries(value)
      .map(([k, v]) => `${k}: ${formatParameterValue(v)}`)
      .join(' · ');
  }
  return String(value);
}
